        ...
        if (Util.hasPermission(sender, "kalesutilities.mute")) {
            if (args.length > 0) {
                    ...
                    try {
                        ...
                        Util.broadcastMessage(KalesUtilities.Instance.config.getString("messages.unmute").replace("{player}", args[0]).replace("{moderator}", Util.getPlayerName(sender)));
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
            } else {
                Util.sendMessage(sender, KalesUtilities.Instance.config.getString("messages.usage").replace("{usage}", KalesUtilities.Instance.getCommand("unmute").getUsage()));
            }
        } else {
