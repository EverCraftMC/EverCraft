    ...
    public void run(CommandSender sender, String label, String[] args) {
        if (args.length > 0) {
            if (Util.hasPermission(sender, "kalesutilities.commands.staff.sudo")) {
                ...
                if (player != null) {
                    Util.sendMessage(player, SpigotPlugin.Instance.config.getString("config.about"));
                } else {
            ...
            Util.sendMessage(sender, SpigotPlugin.Instance.config.getString("config.about"));
        }
    }
}