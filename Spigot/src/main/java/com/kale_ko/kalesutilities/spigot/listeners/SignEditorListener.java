...
public class SignEditorListener implements Listener {
    ...
    public void onSignClick(PlayerInteractEvent event) {
        if (event.getClickedBlock() != null && (event.getClickedBlock().getType() == Material.OAK_SIGN
            ...
            if (event.getAction() == Action.RIGHT_CLICK_BLOCK && event.getPlayer().getInventory().getItemInMainHand().getType() == Material.AIR && event.getPlayer().isSneaking() && Util.hasPermission(event.getPlayer(), "kalesutilities.features.editsigns")) {
