...
public class NicknameCommand extends SpigotCommand {
    public NicknameCommand(String name, String description, List<String> aliases, String usage, String permission) {
    ...
    public boolean execute(CommandSender sender, String label, String[] args) {
        if (args.length > 1) {
            if (Util.hasPermission(sender, "kalesutilities.commands.staff.sudo")) {
                ...
                if (player != null) {
                    SpigotPlugin.Instance.players.set(player.getPlayer().getName() + ".nickname", args[1]);

                    Util.sendMessage(sender, SpigotPlugin.Instance.config.getString("messages.setnickname"));
            ...
            if (sender instanceof Player player) {
                SpigotPlugin.Instance.players.set(player.getPlayer().getName() + ".nickname", args[0]);
            ...
            if (sender instanceof Player player) {
                SpigotPlugin.Instance.players.set(player.getPlayer().getName() + ".nickname", player.getName());
            ...
            Util.sendMessage(sender, SpigotPlugin.Instance.config.getString("messages.usage").replace("{usage}", SpigotPlugin.Instance.getCommand("nickname").getUsage()));
        }
