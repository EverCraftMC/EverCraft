...
public class StaffCommand extends SpigotCommand {
    public StaffCommand(String name, String description, List<String> aliases, String usage, String permission) {
    ...
    public void run(CommandSender sender, String label, String[] args) {
        if (args.length > 0) {
            if (Util.hasPermission(sender, "evercraft.commands.staff.sudo")) {
                ...
                if (player != null) {
                    Util.sendMessage(player, SpigotPlugin.Instance.config.getString("config.staff"));
                } else {
            ...
            Util.sendMessage(sender, SpigotPlugin.Instance.config.getString("config.staff"));
        }
    }
}