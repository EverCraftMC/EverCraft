    ...
    public void run(CommandSender sender, String[] args) {
        if (sender instanceof Player player) {
            ...
            if (args.length > 0 && args[0].equalsIgnoreCase("precise")) {
                ...
                pig = (Pig) player.getWorld().spawnEntity(player.getLocation().toBlockLocation().add(0.5, player.getWorld().getBlockAt(player.getLocation().toBlockLocation().add(0, -1, 0)).getType().getKey().getKey().endsWith("_stairs") || player.getWorld().getBlockAt(player.getLocation().toBlockLocation().add(0, -1, 0)).getType().getKey().getKey().endsWith("_slab") || player.getWorld().getBlockAt(player.getLocation().toBlockLocation().add(0, -1, 0)).getType().getKey().getKey().endsWith("_bed") ? -1.5 : (player.getWorld().getBlockAt(player.getLocation().toBlockLocation()).getType().getKey().getKey().endsWith("_stairs") || player.getWorld().getBlockAt(player.getLocation().toBlockLocation()).getType().getKey().getKey().endsWith("_slab") || player.getWorld().getBlockAt(player.getLocation().toBlockLocation()).getType().getKey().getKey().endsWith("_bed") ? -0.5 : -1), 0.5), EntityType.PIG);
            }
