...
public class ChestProtectionListener extends SpigotListener {
    ...
    public void onChestOpen(PlayerInteractEvent event) {
        if (event.getAction() == Action.RIGHT_CLICK_BLOCK && SpigotMain.getInstance().getChests().get().blocks.containsKey(event.getClickedBlock().getX() + "," + event.getClickedBlock().getY() + "," + event.getClickedBlock().getZ() + "," + event.getClickedBlock().getWorld().getName())) {
            if ((event.getPlayer().isOp() || event.getPlayer().hasPermission("evercraft.chestProtectionBypass")) && (event.getPlayer().getGameMode() == GameMode.CREATIVE || event.getPlayer().getGameMode() == GameMode.SPECTATOR)) {
