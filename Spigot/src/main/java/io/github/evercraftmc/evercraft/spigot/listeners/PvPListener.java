...
public class PvPListener extends SpigotListener {
    ...
    public void onPlayerPvP(EntityDamageByEntityEvent event) {
        if (event.getEntity() instanceof Player player && SpigotMain.getInstance().getPluginConfig().getParsed().passiveEnabled) {
            if (event.getDamager() instanceof Player player2) {
                if (SpigotMain.getInstance().getPluginData().getParsed().players.get(player.getUniqueId().toString()).settings.passive || SpigotMain.getInstance().getPluginData().getParsed().players.get(player2.getUniqueId().toString()).settings.passive) {
