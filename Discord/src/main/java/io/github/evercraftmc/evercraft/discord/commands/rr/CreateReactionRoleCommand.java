...
public class CreateReactionRoleCommand extends Command {
    ...
    public void run(Message message) {
        ...
        BotMain.Instance.getData().reactions.add(new ReactionRole(reactionMessage.getChannel().asTextChannel().getId(), reactionMessage.getId(), ArgsParser.getEmojiArg(message, 2).getAsReactionCode(), ArgsParser.getRoleArg(message, 3).getId()));
